...
from eagle.datasets.old.video_dataset import make_video_data_module
...
class ModelArguments:
    ...
    # Add video
    tune_mm_video_projection: bool = field(default=False)
    video_tower: Optional[str] = field(default=None)
    mm_video_select_layer: Optional[int] = field(default=-1)   # default to the last layer
    mm_video_select_feature: Optional[str] = field(default="patch")
    pretrain_mm_video_projection: Optional[str] = field(default=None)
    mm_video_projector_type: Optional[str] = field(default='linear')
    #END
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # BEGIN
    mm_video_projector_lr: Optional[float] = None
    video_tower_layer_decay: Optional[float] = None
    video_tower_lr: Optional[float] = None
    freeze_mm_video_projection: bool = field(default=False)
    # END
...
def train(attn_implementation=None):
    ...
    if model_args.video_tower is not None:
        ...
        model.config.tune_mm_video_projection = training_args.tune_mm_video_projection = model_args.tune_mm_video_projection
        if model_args.tune_mm_video_projection:
            model.requires_grad_(False)
            for p in model.get_model().mm_video_projector.parameters():
                p.requires_grad = True

        model.config.freeze_mm_video_projection = training_args.freeze_mm_video_projection
        if training_args.freeze_mm_video_projection:
            for p in model.get_model().mm_video_projector.parameters():
                p.requires_grad = False
        
        model.config.mm_video_projector_lr = training_args.mm_video_projector_lr
    ...
    data_module = make_video_data_module(
        video_text_path=data_args.data_path,
        video_path=data_args.video_folder,
        video_pretrain_path=model_args.video_tower,
