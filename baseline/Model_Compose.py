...
def tall_tier_merging(base_model_path, tuned_paths, output_prefix=None, 
    ...
    # 为每个模型重建特定任务信息
    for i, (tuned_path, tuned_sd, task_vector) in enumerate(zip(tuned_paths, tuned_sds, task_vectors)):
        # 确定输出路径
...
tall_tier_merging(
    ...
    output_paths=["checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/merged_image_TALL/model.safetensors", 
                  "checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/merged_video_TALL/model.safetensors"],
    reset_thresh=0.2,
    lambda_scale=.4,
    tall_lambda=1.0,
    remove_keys=[]
