...
def tall_tier_merging(base_model_path, tuned_paths, output_prefix=None, 
    ...
    k = int(len(mtl_vector) * 0.2)  # 后 20% 的数量
    _, bottom_indices = torch.topk(abs_mtl, k, largest=False)  # 设置 largest=False 来获取最小值
    
    # 获取这些位置的值并随机打乱
    bottom_values = mtl_vector[bottom_indices]
    shuffled_values = bottom_values[torch.randperm(len(bottom_values))]
    
    # 将打乱后的值放回原向量
    mtl_vector_shuffled = mtl_vector.clone()
    mtl_vector_shuffled[bottom_indices] = shuffled_values
...
tall_tier_merging(
    ...
    output_paths=["checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/merged_image_low_20_randn/model.safetensors"],
    reset_thresh=0.2,
    lambda_scale=1.,
    tall_lambda=1.0,
    remove_keys=[]
