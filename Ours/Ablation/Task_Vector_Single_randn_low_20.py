...
def tall_tier_merging(base_model_path, tuned_paths, output_prefix=None, 
    ...
    # 获取这些位置的值并加上噪声
    top_values = mtl_vector[bottom_indices]
    alpha = 0.3 * mtl_vector.abs().max()
    noise = torch.randn_like(top_values) * alpha
    noised_values = top_values + noise
    
    # 将加噪声后的值放回原向量
    mtl_vector_noised = mtl_vector.clone()
    mtl_vector_noised[bottom_indices] = noised_values
...
tall_tier_merging(
    ...
    output_paths=["checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/image_low_20_randn_add_noise_0.3/model.safetensors"],
    lambda_scale=1.,
    remove_keys=[]
)
