...
def generate_different_noise_patterns(mtl_vector, top_indices, target_l1_norm):
    """生成多种不同的噪声模式，保持相同的L1范数但产生不同的JS散度"""
    ...
    if noise3.abs().sum() > 0:  # 避免除零
        noise3 = noise3 / noise3.abs().sum() * target_l1_norm
    noise_patterns.append(("sparse", noise3))
    
    # 模式4: 交替符号噪声
    noise4 = torch.ones_like(top_values, dtype=dtype, device=device)
    noise4[::2] = -1  # 交替正负
    noise4 = noise4 / noise4.abs().sum() * target_l1_norm
    ...
    noise5 = noise5 / noise5.abs().sum() * target_l1_norm
    ...
    noise6 = noise6 / noise6.abs().sum() * target_l1_norm
    ...
    noise7 = noise7 / noise7.abs().sum() * target_l1_norm
    ...
    noise8 = noise8 / noise8.abs().sum() * target_l1_norm
    ...
    noise9 = noise9 / noise9.abs().sum() * target_l1_norm
    ...
    noise10 = noise10 / noise10.abs().sum() * target_l1_norm
    ...
    noise11 = noise11 / noise11.abs().sum() * target_l1_norm
    ...
    if noise12.abs().sum() > 0:  # 避免除零
        noise12 = noise12 / noise12.abs().sum() * target_l1_norm
...
def tall_tier_merging_multi_noise(base_model_path, tuned_paths, output_base_dir, 
    ...
    target_l1_norm = reference_noise.abs().sum()
    
    print(f"目标L1范数: {target_l1_norm.item():.6f}")
    
    # 生成4种不同的噪声模式
    noise_patterns = generate_different_noise_patterns(mtl_vector, top_indices, target_l1_norm)
    ...
    for pattern_idx, (pattern_name, noise) in enumerate(noise_patterns):
        ...
        # 验证L1范数
        actual_l1_norm = noise.abs().sum()
        print(f"实际L1范数: {actual_l1_norm.item():.6f}")
...
# 使用示例：生成4个具有相同L1 Loss但不同JS散度的模型
for noise_scale in [0.1]:
    ...
    tall_tier_merging_multi_noise(
        ...
        output_base_dir=f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_{noise_scale}",
        lambda_scale=1.,
        remove_keys=[],
        noise_scale=noise_scale
    )

# 目标L1范数: 1335296.000000

# 处理噪声模式 1: gaussian
# 实际L1范数: 1335296.000000
# JS散度: 0.119141
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js/noise_pattern_1_gaussian/model.safetensors

# 处理噪声模式 2: uniform
# 实际L1范数: 1335296.000000
# JS散度: 0.103516
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js/noise_pattern_2_uniform/model.safetensors

# 处理噪声模式 3: sparse
# 实际L1范数: 1335296.000000
# JS散度: 0.123047
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js/noise_pattern_3_sparse/model.safetensors

# 处理噪声模式 4: alternating
# 实际L1范数: 1335296.000000
# JS散度: 0.070801
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js/noise_pattern_4_alternating/model.safetensors

# 处理噪声模式 5: laplace
# 实际L1范数: 1335296.000000
# JS散度: 0.138672
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_5_laplace/model.safetensors

# 处理噪声模式 6: exponential
# 实际L1范数: 1335296.000000
# JS散度: 0.138672
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_6_exponential/model.safetensors

# 处理噪声模式 7: sinusoidal
# 实际L1范数: 1327104.000000
# JS散度: 0.092285
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_7_sinusoidal/model.safetensors

# 处理噪声模式 8: piecewise_constant
# 实际L1范数: 1343488.000000
# JS散度: 0.039062
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_8_piecewise_constant/model.safetensors

# 处理噪声模式 9: power_law
# 实际L1范数: 1335296.000000
# JS散度: 0.128906
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_9_power_law/model.safetensors

# 处理噪声模式 10: triangle_wave
# 实际L1范数: 1335296.000000
# JS散度: 0.102051
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_10_triangle_wave/model.safetensors

# 处理噪声模式 11: bimodal
# 实际L1范数: 1335296.000000
# JS散度: 0.093262
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_11_bimodal/model.safetensors

# 处理噪声模式 12: random_walk
# 实际L1范数: 1335296.000000
# JS散度: 0.132812
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js1/noise_pattern_12_random_walk/model.safetensors

# 处理噪声模式 0.1
# 目标L1范数: 266240.000000

# 处理噪声模式 1: gaussian
# 实际L1范数: 266240.000000
# JS散度: 0.061523
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_1_gaussian/model.safetensors

# 处理噪声模式 2: uniform
# 实际L1范数: 266240.000000
# JS散度: 0.060547
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_2_uniform/model.safetensors

# 处理噪声模式 3: sparse
# 实际L1范数: 266240.000000
# JS散度: 0.058838
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_3_sparse/model.safetensors

# 处理噪声模式 4: alternating
# 实际L1范数: 266240.000000
# JS散度: 0.059326
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_4_alternating/model.safetensors

# 处理噪声模式 5: laplace
# 实际L1范数: 266240.000000
# JS散度: 0.064453
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_5_laplace/model.safetensors

# 处理噪声模式 6: exponential
# 实际L1范数: 266240.000000
# JS散度: 0.064453
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_6_exponential/model.safetensors

# 处理噪声模式 7: sinusoidal
# 实际L1范数: 266240.000000
# JS散度: 0.060791
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_7_sinusoidal/model.safetensors

# 处理噪声模式 8: piecewise_constant
# 实际L1范数: 266240.000000
# JS散度: 0.081055
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_8_piecewise_constant/model.safetensors

# 处理噪声模式 9: power_law
# 实际L1范数: 266240.000000
# JS散度: 0.064941
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_9_power_law/model.safetensors

# 处理噪声模式 10: triangle_wave
# 实际L1范数: 266240.000000
# JS散度: 0.067383
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_10_triangle_wave/model.safetensors

# 处理噪声模式 11: bimodal
# 实际L1范数: 266240.000000
# JS散度: 0.062012
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_11_bimodal/model.safetensors

# 处理噪声模式 12: random_walk
# 实际L1范数: 266240.000000
# JS散度: 0.052734
# 模型已保存至: checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/same_l1_diff_js_0.1/noise_pattern_12_random_walk/model.safetensors