...
def tall_tier_merging(base_model_path, tuned_paths, output_prefix=None, 
    ...
    sorted_indices = torch.argsort(abs_mtl)  # 从小到大排序
    n = len(mtl_vector)
    start = int(n * 0.6)
    end = int(n * 0.8)
    ...
    # 获取这些位置的值并加上噪声
    mid_values = mtl_vector[mid_indices]
    alpha = noise_scale * mtl_vector.abs().max()
    noise = torch.randn_like(mid_values) * alpha
    noised_values = mid_values + noise

    # 将加噪声后的值放回原向量
    mtl_vector_noised = mtl_vector.clone()
    mtl_vector_noised[mid_indices] = noised_values
...
# 使用示例
for noise_scale in [0.1, 0.2, 0.3]:
    tall_tier_merging(
        ...
        output_paths=[f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/image_mid_20_randn_add_noise_{noise_scale}/model.safetensors"],
        lambda_scale=1.,
        remove_keys=[],
