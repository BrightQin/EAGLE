...
def optimize_task_vectors_entropy(task_vectors, learning_rate=5e-5, max_iterations=50, temperature=2e-4, eps=1e-8, alpha=0.5):
    ...
    def compute_loss(mtl_vec, lambda_js=0.5):  # 添加lambda_js参数来控制JS散度中的权重
        ...
        for i in tqdm(range(n_tasks), desc="计算损失"):
            ...
            js_loss += chunk_js_loss
        
        js_loss = js_loss / n_tasks
        
        # 计算元素级别的L2损失 - 使mtl向量的每个元素幅度接近每个任务向量对应元素的幅度
        element_mag_loss = torch.tensor(0.0).to(device)
        for i in range(n_tasks):
            task_mag = task_vectors[i].abs()  # 第i个任务向量的元素幅度
            l2_loss = torch.mean((mtl_vec.abs() - task_mag) ** 2)  # L2损失
            element_mag_loss += l2_loss
        element_mag_loss = element_mag_loss / n_tasks  # 对所有任务求平均
        element_mag_loss = element_mag_loss * 1e7
        print(f"mtl平均幅度: {mtl_vec.abs().mean().item()}, 任务向量平均幅度: {task_magnitudes.mean().item()}, 元素L2损失: {element_mag_loss.item()}")
...
#     output_paths=[
#         f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/weight_averaging_image_video_increamental_js_{lambda_scale}/model.safetensors",
#         f"checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/weight_averaging_image_video_increamental_js_{lambda_scale}/model.safetensors"
#     ]
#     tall_tier_merging(
...
for lambda_scale in [round(x, 1) for x in torch.arange(1.0, 1.01, 0.1).tolist()]:
    print(f"当前 lambda_scale: {lambda_scale}")
    output_paths=[
        f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/weight_averaging_image_video_audio_increamental_js_l2_{lambda_scale}/model.safetensors",
        f"checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/weight_averaging_image_video_audio_increamental_js_l2_{lambda_scale}/model.safetensors",
        f"checkpoints/Incremental/Audio/finetune-video-llama3.2-1b/finetune/pr_llm/finetune-audio-llama3.2-1b/weight_averaging_image_video_audio_increamental_js_l2_{lambda_scale}/model.safetensors"
    ]
    tall_tier_merging(
        base_model_path="checkpoints/Baseline/Images/finetune/pr/finetune-eagle-x1-llama3.2-1b-image_L/model.safetensors",
        tuned_paths=[
