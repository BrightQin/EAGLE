...
def optimize_task_vectors_entropy(task_vectors, learning_rate=5e-5, max_iterations=100, 
                                 tolerance=1e-6, temperature=5e-4, eps=1e-8):
    """
    优化任务向量的融合，直接最小化信息熵损失
    ...
    # 初始化多任务向量为任务向量的平均值
    mtl_vector = task_vectors.mean(dim=0).clone().detach().requires_grad_(True)
    ...
    def to_prob_dist(vector, temp=temperature):
        ...
        chunk_size = min(10000, vector.shape[0])  # 选择合适的分块大小
    ...
    # 计算KL散度损失
    def compute_kl_loss(mtl_vec):
        mtl_prob = to_prob_dist(mtl_vec)
        total_kl_loss = 0
        
        for i in tqdm(range(n_tasks), desc="计算KL散度"):
            task_prob = task_probs[i]  # 使用预计算的概率分布
            
            # 分块计算KL散度
            chunk_size = min(10000, mtl_prob.shape[0])
            ...
            total_kl_loss += chunk_kl_loss
            
        return total_kl_loss / n_tasks
    ...
    for iteration in range(max_iterations):
        ...
        # 计算损失
        loss = compute_kl_loss(mtl_vector)
        ...
        if (iteration + 1) % 1 == 0:
            print(f"迭代 {iteration + 1}/{max_iterations}, 损失: {current_loss:.6f}")
                
    # 返回优化后的多任务向量
    return mtl_vector.detach()
...
def tall_tier_merging(base_model_path, tuned_paths, output_prefix=None, 
    ...
    # 使用优化的方法计算多任务向量
    task_vectors = task_vectors[:, :2000000]
    mtl_vector = optimize_task_vectors_entropy(task_vectors)
...
tall_tier_merging(
    ...
    output_paths=["checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/merged_image_TALL_our/model.safetensors", 
                  "checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/merged_video_TALL_our/model.safetensors"],
    reset_thresh=0.2,
    lambda_scale=1.,
    tall_lambda=1.0,
    remove_keys=[]
