...
def optimize_task_vectors_entropy(task_vectors, learning_rate=5e-5, max_iterations=100, temperature=2e-4, eps=1e-8, alpha=0.5):
    """
    ...
    # mtl_vector = (torch.zeros_like(task_vectors) + 1e-5).mean(dim=0).clone().detach().requires_grad_(True).to(device)
...
for lambda_scale in [round(x, 1) for x in torch.arange(1.0, 1.01, 0.1).tolist()]:
    ...
    output_paths=[
        f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/weight_averaging_image_video_increamental_js_without_js_zeros_l2_4000_mean_{lambda_scale}/model.safetensors",
        f"checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/weight_averaging_image_video_increamental_js_without_js_zeros_l2_4000_mean_{lambda_scale}/model.safetensors"
    ]
    tall_tier_merging(
        ...
        tuned_paths=[
            ...
            "checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/checkpoint-4000/model.safetensors"
        ],
