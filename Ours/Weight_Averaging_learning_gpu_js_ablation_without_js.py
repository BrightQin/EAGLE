...
for lambda_scale in [round(x, 1) for x in torch.arange(1.0, 1.01, 0.1).tolist()]:
    ...
    output_paths=[
        f"checkpoints/Baseline/Images/finetune/pr_llm/finetune-eagle-x1-llama3.2-1b-image_L/weight_averaging_image_video_increamental_js_without_js_zeros_{lambda_scale}/model.safetensors",
        f"checkpoints/Incremental/Video/finetune-eagle-x1-llama3.2-1b-image_L/finetune/pr_llm/finetune-video-llama3.2-1b/weight_averaging_image_video_increamental_js_without_js_zeros_{lambda_scale}/model.safetensors"
    ]
    tall_tier_merging(
